...
# 🔑 Разделяем ключи данных и виджета для обхода политики Streamlit
...
# ⚠️ Предупреждение для неопределённых
current_df = st.session_state[DATA_KEY]
if "🚦 Статус" in current_df.columns:
    undetected = current_df[current_df["🚦 Статус"].str.contains("🔴", na=False)]
    if not undetected.empty:
        st.warning(f"🔴 У этих резидентов пол не определён автоматически, проверьте вручную: {', '.join(undetected['Имя'])}")
...
edited_df = st.data_editor(
    ...
    num_rows="dynamic",
    key=EDITOR_KEY  # Отличается от DATA_KEY → ошибка исчезает
)
st.session_state[DATA_KEY] = edited_df  # Безопасная синхронизация
...
if run:
    ...
    try:
        ...
        # 📥 Экспорт в Excel
        if res.groups:
            excel_data = [{"Группа": f"Группа {i} ({len(g)} чел.)", "Резиденты": ", ".join(g)} for i, g in enumerate(res.groups, 1)]
