...
if TABLE_KEY not in st.session_state:
    ...
    if "Роль" in st.session_state[TABLE_KEY].columns:
        st.session_state[TABLE_KEY]["Роль"] = st.session_state[TABLE_KEY]["Роль"].replace(role_map)
...
# Передаём копию, чтобы обойти проверку политик Streamlit.
# Виджет САМ синхронизирует данные в st.session_state[TABLE_KEY] благодаря key=
st.data_editor(
    st.session_state[TABLE_KEY].copy(),
    column_config={
...
# ⛔ НИКАКОЙ РУЧНОЙ СИНХРОНИЗАЦИИ ЗДЕСЬ! Streamlit делает это автоматически через key=
...
if run:
    # Читаем напрямую из состояния виджета
