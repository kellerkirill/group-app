...
# 🔑 РАЗДЕЛЯЕМ КЛЮЧИ: один для данных, другой для виджета
...
# 📝 Таблица резидентов (КРИТИЧЕСКИЙ ФИКС)
st.subheader("📝 Резиденты")

# На первом рендере передаём данные. На последующих — пустой шаблон,
# чтобы обойти StreamlitValueAssignmentNotAllowedError
editor_data = st.session_state[DATA_KEY] if WIDGET_KEY not in st.session_state else pd.DataFrame(columns=COLUMNS)
...
# Безопасная синхронизация (ключи разные → политика не срабатывает)
